 ...
 * ######################################################
 * */
_private s16 create(void (*task)(void),void (*error_hook)(void),E_TaskPriority priority);
/*
 * ######################################################
...
s16 create_process(void (*task)(void),void (*error_hook)(void),E_TaskPriority priority)
{
	/*returns -1 in case of error or a number >0 that represents the process id.*/
	u8 errorNumber=EOK;

	enable_protection();
	if(create(task,error_hook,priority)!=0){
		errorNumber=ETSOF;
	}
	disable_protection();
...
s16 create(void (*task)(void),void (*error_hook)(void),E_TaskPriority priority)
{
	u8 taskIndex=0;
